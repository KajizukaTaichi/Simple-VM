    ...
    for code in asm.split("\n") {
        ...
        if let Mode::Program = mode {
            result.push(match args[0] {
                ...
                "JUMP" | "jump" => Instruction::JumpIfZero(args[1].trim().parse().unwrap_or(0)),
                "LOAD" | "load" => Instruction::Load(args[1].trim().parse().unwrap_or(0)),
                "STORE" | "store" => Instruction::Store(args[1].trim().parse().unwrap_or(0)),
